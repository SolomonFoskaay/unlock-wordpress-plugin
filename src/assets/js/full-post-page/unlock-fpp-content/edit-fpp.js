...
export default function Edit({ attributes, setAttributes }) {
  ...
  const showPostContent = () => {
    ...
    const locks = meta['unlock-protocol'] ? meta['unlock-protocol'].locks : [];
  
    useEffect(() => {
      if (!locksValid(locks) && !isSavingPost) {
    ...
    return (
      <div {...useBlockProps()}>
        { locksValid(locks) ? <InnerBlocks /> : lockWarning() }
      </div>
    );
  };
  ...
  return (
    <>
      <div {...useBlockProps()}>
        ...
        {locksValid(locks) ? showPostContent() : lockWarning()}
      </div>
    </>
  );
}
